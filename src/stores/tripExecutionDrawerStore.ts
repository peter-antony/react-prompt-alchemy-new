...
interface TripExecutionDrawerStore {
  ...
  getConsignmentByIndex: (legId: string, index: number) => Consignment | undefined;
}
...
export const useTripExecutionDrawerStore = create<TripExecutionDrawerStore>(
  (set, get) => ({
    legs: [],
    selectedLegId: null,
    ...
    removeLeg: (legId) => {
      ...
      set({
        legs,
        selectedLegId:
          selectedLegId === legId ? legs[0]?.id || null : selectedLegId,
      });
    },
      ...
      set((state) => ({
        legs: state.legs.map((leg) =>
          leg.id === legId
            ? {
                ...leg,
                consignments: leg.consignments.some(
                  (c) => c.id === consignment.id
                )
                  ? leg.consignments.map((c) =>
                      c.id === consignment.id ? consignment : c
                    )
      ...
      set((state) => ({
        legs: state.legs.map((leg) =>
          leg.id === legId
            ? {
                ...leg,
                transshipments: leg.transshipments.some(
                  (t) => t.id === transshipment.id
                )
                  ? leg.transshipments.map((t) =>
                      t.id === transshipment.id ? transshipment : t
                    )
    ...
    loadLegsFromAPI: () => {
      try {
        ...
        if (apiLegDetails && apiLegDetails.length > 0) {
          const transformedLegs = apiLegDetails.map(
            (apiLeg: any, index: number) =>
          ...
          set({
            ...
            selectedLegId:
              transformedLegs.length > 0 ? transformedLegs[0].id : null,
          });
        } else {
    ...
    getConsignments: (selectedLegId: any) => {
      try {
        const consignments = get().legs.filter(
          (leg) => leg.id === selectedLegId
        );
    ...
    getConsignmentByIndex: (
      ...
      try {
        ...
        const cons = leg.consignments || [];
        return cons[consignmentIndex] || null;
      } catch (error) {
        console.error("Error getting consignment by index:", error);
        return null;
      }
    },
  })
