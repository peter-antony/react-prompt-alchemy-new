...
saveWorkOrder: async () => {
  ...
  const formatCreationTime = (val: any): string => {
    if (!val) return "";
    const str = String(val);
    const timePart = str.includes(" ") ? str.split(" ")[1] : str;
    const withoutMillis = timePart.split(".")[0];
  ...
  const rawWorkOrder = useWorkOrderStore.getState().workOrder;
  const workOrder = rawWorkOrder
    ? {
        ...rawWorkOrder,
        Header: {
          ...rawWorkOrder.Header,
          CreationDateTime: formatCreationTime(rawWorkOrder.Header?.CreationDateTime),
          ModificationDateTime: formatCreationTime(rawWorkOrder.Header?.ModificationDateTime),
        },
      }
    : rawWorkOrder;
  try {
