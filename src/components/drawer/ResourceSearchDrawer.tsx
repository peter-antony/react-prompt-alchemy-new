...
interface ResourceSearchDrawerProps {
  isOpen: boolean;
  onClose: () => void;
...
export const ResourceSearchDrawer: React.FC<ResourceSearchDrawerProps> = ({
  isOpen,
  onClose,
  onAddResource,
  resourceType,
...
// checking with static equipment id- working
  ...
  const handleSaveResource = async () => {
    ...
    console.log('[Save Resource] selectedRowObjects:', selectedRowObjects);
    
    // Format all selected resources into an array
    const localData = selectedRowObjects.map((rowObj) => {
    ...
    // Pass the formatted data array to parent component
    if (onAddResource && localData.length > 0) {
      onAddResource(localData);
      onClose();
    }
  };
