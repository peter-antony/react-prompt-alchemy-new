...
const GridResourceDetails: React.FC<CardDetailsProps> = ({ data, isEditQuickOrder, passedQuickUniqueID }) => {
  ...
  const [apiStatus, setApiStatus] = useState<'idle' | 'loading' | 'done'>('idle');
  ...
  return (
    <div>
      <div className="w-80 mb-3">
        <div className="relative flex rounded-md overflow-hidden bg-white text-sm">
          <Select
            value={selectedValue}
          ...
          >
            ...
            <SelectContent className="bg-white border shadow-lg z-50">
              ...
              {filteredOrders.map((item: any, idx: number) => (
                <SelectItem key={item.QuickUniqueID || idx} value={item.QuickUniqueID} className="text-xs">
                  {`${item.QuickOrderNo} || ${item.Status.value}`}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {filteredResourceGroups && filteredResourceGroups.length > 0 ? (
            ...
            <div key={'Resource' + index} className="bg-white rounded-lg border border-[#EAECF0] px-4 py-4 relative">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-4 text-sm mb-3">
                ...
                <div className="flex items-center gap-2 text-gray-700 col-span-2 text-xs relative group">
                  ...
                  <span className="truncate cursor-pointer">{item?.OperationalDetails?.OperationalLocationDesc}</span>
                  <div className="absolute left-0 top-4 z-30 hidden group-hover:block min-w-[180px] max-w-xs bg-[#344054] text-white rounded-lg shadow-xl border border-gray-200 text-xs">
                    <div className="py-2 pl-[14px] pr-[14px]">
                      {/* <div className="font-semibold mb-1">From and To Date</div> */}
          ...
          <div className="col-span-2 flex flex-col items-center justify-center py-12">
            <div className="text-lg font-semibold text-gray-500 mb-2">Data not found</div>
            <div className="text-sm text-gray-400">No resource group details available for the selected item.</div>
          </div>
        )}
        <SideDrawer
