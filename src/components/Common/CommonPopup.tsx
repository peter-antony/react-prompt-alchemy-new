...
const CommonPopup: React.FC<CommonPopupProps> = ({
    ...
    const validateAndSubmit = () => {
        const requiredEmpty = fields.filter(f => f.required && (!f.value || String(f.value).trim() === ""));
        if (requiredEmpty.length > 0) {
            const firstLabel = requiredEmpty[0]?.label ?? "Required field";
            setValidationError(`${firstLabel} is required.`);
            return;
        }
        setValidationError(null);
        onSubmit(fields);
                    ...
                    >
                        ...
                        {fields.map(field => (
                            <div key={field.name} className="flex flex-col gap-1">
                                ...
                                {field.type === "select" ? (
                                    <select
                                        className={`border rounded px-3 py-2 text-sm text-gray-600 w-full ${field.required && !field.value ? "border-red-400" : ""}`}
                                        value={field.value}
                                    ...
                                    <input
                                        type="text"
                                        className={`border rounded px-3 py-2 text-sm ${field.required && !field.value ? "border-red-400" : ""}`}
