...
export function DynamicLazySelect1({
    fetchOptions,
    value,
    onChange,
    ...
    const handleClear = (e: React.MouseEvent) => {
        ...
        setIsOpen(false); // Close the popover to prevent focus issues
    };
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            ...
            } else if (allowNewEntry && searchTerm.length >= minSearchLength) {
                // If no options but new entry is allowed and meets minimum length
