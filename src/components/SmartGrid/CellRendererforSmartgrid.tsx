...
import { LazySelectWithNewEntry } from './LazySelectWithNewEntry';
...
export const CellRendererforSmartgrid: React.FC<CellRendererProps> = ({
  value,
  row,
  column,
  rowIndex,
  ...
  // LazySelect renderer
  const renderLazySelect = () => {
    // If not editable, just show the value as text
    if (!isEditable) {
      return <span className="truncate text-[13px]" title={String(value || '')}>{value !== null && value !== undefined ? String(value) : ''}</span>;
    }

    // If editable and has fetchOptions, show LazySelectWithNewEntry component
    if (column.fetchOptions) {
      return (
        <LazySelectWithNewEntry
          fetchOptions={column.fetchOptions}
          value={value || ''}
          onChange={(newValue) => onEdit(rowIndex, column.key, newValue)}
          placeholder={column.placeholder || 'Select...'}
          allowNewEntry={column.allowNewEntry || false}
          onNewEntry={(newValue) => onEdit(rowIndex, column.key, newValue)}
          className="text-[13px] w-full"
          hideSearch={column.hideSearch}
          disableLazyLoading={column.disableLazyLoading}
          returnType={column.returnType}
          minSearchLength={column.minSearchLength || 1}
        />
      );
    }

    // Fallback to regular select if options are provided but no fetchOptions
    if (column.options) {
      return (
        <select
          value={value || ''}
    ...
    // If no options or fetchOptions, just show as text
    return <span className="truncate text-[13px]" title={String(value || '')}>{value !== null && value !== undefined ? String(value) : ''}</span>;
  };
