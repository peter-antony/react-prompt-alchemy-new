...
import { CimTemplateSearchCriteria, ITemplateCriteria } from "@/constants/cimcuvSearchCriteria";
import { useFilterStore } from "@/stores/filterStore";
import { Button } from "@/components/ui/button";
import { dateFormatter } from "@/utils/formatter";
import { CimCuvService } from "@/api/services/CimCuvService";

export const TemplateSearchHub = () => {
  const [searchParams] = useSearchParams();
  const gridId = "CIM-template-hub"; // same id you pass to SmartGridWithGrouping
  ...
  const fetchCimCuvTemplateData = async () => {
    ...
    try {
      ...
      const response: any = await CimCuvService.getCimCuvTemplateHub({ searchCriteria });
      const parsedResponse = JSON.parse(response?.data.ResponseData || "{}");
      const data = parsedResponse.Templates;
    ...
    const init = async () => {
      try {
        ...
        try {
          ...
          console.log('CimTemplateHub: Server-side filter personalization response:', serverFilterPersonalizationResponse);
  ...
  const handleLinkClick = (value: any, columnKey: any) => {
    ...
    if (columnKey === 'TemplateID') {
      navigate(`/create-template?id=${value.TemplateID}`);
    }
  };
  ...
  const gridConfigurableButtons: ConfigurableButtonConfig[] = [
    {
      ...
      tooltipTitle: "Create a new template",
      showDropdown: false,
  ...
  const buildSearchCriteria: any = (latestFilters: any) => {
    const criteria: ITemplateCriteria = { ...CimTemplateSearchCriteria };
  ...
  const handleServerSideSearch = async () => {
    ...
    try {
      gridState.setLoading(true);
      setApiStatus('loading');
      const response: any = await CimCuvService.getCimCuvTemplateHub({
