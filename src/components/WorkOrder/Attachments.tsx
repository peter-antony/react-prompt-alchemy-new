...
const Attachments = ({
  ...
  useEffect(() => {
    ...
    const fetchLegBehaviours = fetchData("Attachment File Category WO Init");
  ...
  const getAttachments = async () => {
    let id=workId?workId:tripId;
    try{
    const response = await workOrderService.getAttachments(id);
  ...
  const handleUpload = async (files: StagedFile[]) => {
    ...
    try {
      for (const file of files) {
        ...
        // Send to API with FormData containing binary file
        const response: any = await workOrderService.saveAttachments(
          fileData,
          workId?workId:tripId
        );
  ...
  const handleDelete = async (fileId: any) => {
    // Simulate API call
    console.log("Deleting WorkOrder file:", fileId);
    console.log(
      "isWorkOrderAttachment IN WORKORDER ATTACHMENT:",
      isWorkOrderAttachment
    );
    const fileData = {
